...
def buildTree(preorder, inorder):
    ...
    root.left = buildTree(preorder[1 : root_index + 1], inorder[ : root_index])  # 构建左子树
    root.right = buildTree(preorder[root_index + 1: ], inorder[root_index + 1 : ])  # 构建右子树
    return root


def breath_travel(preorder, inorder):
...
if __name__ == '__main__':
    print(breath_travel([3,9,20,15,7], [9,3,15,20,7]))
    print(breath_travel([], []))
